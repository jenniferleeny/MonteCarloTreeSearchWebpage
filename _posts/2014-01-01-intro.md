...
\item Implement a sequential version of MCTS as well as a testing environment
\item	At random rollout parallelization to MCTS
\item	At iteration parallelization to MCTS
\item	Stretch goals:
\item	Make MCTS more cache efficient
\item	Use MCTS to solve games other than Go (such as Chess and Checkers)
\item	Demo plans:
\item	Compare the win-rate/Elo rating of the algorithm with different amounts of parallelism.
\item	Have an interactive demo where people can play against the AI and view the AI playing against itself
\item	Goals:
\item	Have the AI?s performance be able to scale well with respect to the amount of compute power given to it. Specifically, MCTS with 2N processors should be able to beat MCTS with N processors consistently for reasonably large N.
\end{enumerate}
# Platform
